...
var PARENT_FIELDS = ['classNames', 'type'];
var PARENT_FIELDS_LENGTH = PARENT_FIELDS.length;
...
var MaskedField = _react2.default.createClass({
  ...
  render: function render() {
    ...
    if (this.props.mask) {
      ...
      for (var i = 0; i < PARENT_FIELDS_LENGTH; i++) {
        var field = PARENT_FIELDS[i];
    ...
    return _react2.default.createElement('input', _extends({ ref: function ref(c) {
        return _this._input = c;
      } }, props));
  },
  _getSelection: function _getSelection() {
    if (this._isMounted) {
      return (0, _SelectionUtils.getSelection)(this._input);
    } else {
  ...
  _setSelection: function _setSelection(start) {
    ...
    if (this._input === document.activeElement) {
      (0, _SelectionUtils.setSelection)(this._input, start, end);
    }
  },
  ...
  _isBufferFull: function _isBufferFull() {
    ...
    return this._buffer.every(function (char, idx) {
      return !_this3._getPattern(idx) || char !== BLANK_CHAR;
    });
  },
  ...
  _handleFocus: function _handleFocus(e) {
    ...
    setTimeout(function () {
      return _this4._setSelection(_this4._cursorPos);
    }, 0);
